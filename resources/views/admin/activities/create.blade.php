            ...
            <form action="{{ route('admin.activities.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('intensity_level') is-invalid @enderror" id="intensity_level"
                        ...
                        <option value="high" {{ old('intensity_level') == 'high' ? 'selected' : '' }}>High</option>
                        <option value="moderate" {{ old('intensity_level') == 'moderate' ? 'selected' : '' }}>Moderate
                        </option>
                        <option value="low" {{ old('intensity_level') == 'low' ? 'selected' : '' }}>Low</option>
                    </select>
                ...
                <div class="mb-3 form-check">
                    <input type="checkbox" class="form-check-input @error('active') is-invalid @enderror" id="active"
                        name="active" value="1" {{ old('active', '1') == '1' ? 'checked' : '' }}>
